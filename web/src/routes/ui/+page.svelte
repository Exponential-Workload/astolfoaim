<script lang="ts">
  ...
  const actions: Record<action, (value: any) => string> = {
    ...
    zeroprecisionrecursioncount: (value: number) =>
      `api.zeroprecisionrecursioncount=${value};`,
...
Try increasing this if your pointer (NOT CIRCLE) seems a bit more delayed than it should be.`}
          ...
          >
            <label
              for="zeroprecisionrecursioncount"
            ...
            <input
              type="range"
              name="zeroprecisionrecursioncount"
              id="zeroprecisionrecursioncount"
              data-action="zeroprecisionrecursioncount"
              min="0"
              max="8"
              step="1"
              bind:value={profile.zeroprecisionrecursioncount}
              on:change={changed}
            />
            <br />
            <small class="infothing"
              >{profile.zeroprecisionrecursioncount} recursion{profile.zeroprecisionrecursioncount ===
              1
                ? ''
                : 's'}</small
            >
